...
function animateLable() {
  document.addEventListener(inputTypes.mouseDown, (e) => {
    for (let i = 0; i < formLabels.length; i++) {
      let label = formLabels[i];
      if (label.contains(e.target)) {
        label.classList.add(config.clickedFormLabel);
        label.classList.remove(config.formLabel);
      } else {
        label.classList.remove(config.clickedFormLabel);
        label.classList.add(config.formLabel);
      }
    }
  });
}
